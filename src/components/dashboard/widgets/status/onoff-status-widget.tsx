...
export default function OnOffStatusWidget({
  data,
  size,
}: OnOffStatusWidgetProps) {
  const isOn = useMemo(() => {
    if (!data || data.length === 0) return false
    const latest = data[0]
    return (
      latest.data.state === true ||
        ...
        <div className="flex flex-col">
          <div className="flex items-center mb-4">
            ...
            <span className="ml-2 text-xs text-muted-foreground">
              {data.length > 0 &&
                `Última atualização ${dayJs(data[0].receivedAt).fromNow()}`}
            </span>
          </div>

          {size === 'LARGE' && (
            <div className="mt-4">
              <h4 className="text-sm font-medium mb-2">Atividades Recentes</h4>
